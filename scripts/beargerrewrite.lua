    ...
    while not target do
        if not target then
            for k,v in pairs(ents) do
                if v and ((honeypass and v:HasTag("honeyed")) or not honeypass) and v:IsOnValidGround() and inst.components.eater:CanEat(v) and (v.components.inventoryitem and not v.components.inventoryitem:IsHeld()) then
                    target = v
                    break
                end
            end
...
local BeargerBrainNew = Class(Brain, function(self, inst)
...
local function RetargetFn(inst)
    ...
    if inst.last_eat_time and (GetTime() - inst.last_eat_time) > TUNING.BEARGER_DISGRUNTLE_TIME then
        return FindEntity(inst, TARGET_DIST*5, 
        function(guy) 
            return inst.components.combat:CanTarget(guy)
               and not guy:HasTag("prey")
               and not guy:HasTag("smallcreature")
               and not guy:HasTag("putin")
        end)
    else
...
function BeargerBrainNew:OnStart()
    ...
    local root =
        PriorityNode(
        {
            ...
            ChaseAndAttack(self.inst, 20, 60, nil, RetargetFn, true),
